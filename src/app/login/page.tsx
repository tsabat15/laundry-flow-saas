...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Proses validasi akun ke Firebase
      await signInWithEmailAndPassword(auth, email, password);
      router.push("/dashboard"); // Arahkan ke dasbor kasir
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-display">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-100 p-8 sm:p-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="flex flex-col items-center justify-center mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1 text-center">Masuk ke Dasbor Kasir Anda.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" placeholder="••••••••" />
          </div>
